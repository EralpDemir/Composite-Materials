function OPT_outputlp(nx, ny, Lx, ratio, el_vec_prop)
...
% Lx: size of the laminate in x-direction
...
x = linspace(0,Lx,nx);
y = linspace(0,Lx/ratio,ny);
imagesc(x,y,el_vec_prop_flpd)
colormap(jet(256))

%%%%%%%%%%%%%%%% Plotting mesh lines %%%%%%%%%%%%%%%%%%%%%%%%%%
dx = Lx/(nx-1); dy = Lx/ratio/(ny-1);
for i = -dx/2:dx:Lx+dx/2
    line([i i], [-dy/2 Lx/ratio+dy/2], 'Color','k');
end
for i = -dy/2:dy:Lx/ratio+dy/2
    line([-dx/2 Lx+dy/2], [i i], 'Color','k');
end
set(gca,'xtick',[],'ytick',[]);
