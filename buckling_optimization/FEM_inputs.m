...
qL_u = zeros(numel,nnpe);
Fipedlu_ElNod = zeros(numel,nnpe);

% Apply force on the right edge
Fipedlu_ElNod(nx:nx:nx*ny,2) = 1;
qL_u(nx:nx:nx*ny,2) = -1;

% Apply force on the left edge
Fipedlu_ElNod(1:nx:nx*ny,4) = 1;
qL_u(1:nx:nx*ny,4) = 1;


qL_v = zeros(numel,nnpe);
...
% BC Inputs (simply-supported bottom and top / clamped left and right) 
%
...
dofBCs_b = [1, 2, 3, 4  ...
    dof_b*(nx+1)-dof_b+1, ...
    dof_b*(nx+1)*(ny+1)-dof_b+1,...
    dof_b*(ny*(nx+1)+1)-dof_b+1, ];


% plot(crds(1,1),crds(1,2),'r*')
% plot(crds(nx+1,1),crds(nx+1,2),'r*')
% plot(crds((nx+1)*(ny+1),1),crds((nx+1)*(ny+1),2),'r*')
% plot(crds(ny*(nx+1)+1,1),crds(ny*(nx+1)+1,2),'r*')


% Along bottom edge: simply support (w=0)
for i = 2:1:nx
    dofBCs_b = [dofBCs_b, dof_b*(i-1)+1];
%     plot(crds(i,1),crds(i,2),'k.')
end
...
% % Along bottom edge: simply support (w=0)
% for i = 2:1:nx
%     dofBCs_b = [dofBCs_b, dof_b*(i-1)+1, dof_b*(i-1)+3, dof_b*(i-1)+4];
% %     plot(crds(i,1),crds(i,2),'k.')
% end
% 
% % Along left edge: simply support (w=0)
% for i = nx+2:nx+1:(ny-1)*(nx+1)+1
%     dofBCs_b = [dofBCs_b, dof_b*(i-1)+1, dof_b*(i-1)+2, dof_b*(i-1)+4];
% %     plot(crds(i,1),crds(i,2),'k.')
% end
% 
% % Along top edge:  simply support (w=0)
...
% Membrane BCs
% Dofs for reaction forces
dofBCs_m_bottom=[];
dofBCs_m_top = [];

dofBCs_m=[];

% Lower left corner along y-direction is fixed
dofBCs_m = [dofBCs_m, 2];

% % Along left edge: fixed - x (x=0)
% for i = 1:nx+1:ny*(nx+1)+1
% %     plot(crds(i,1),crds(i,2),'r.')
%     dofBCs_m = [dofBCs_m, dof_m*(i-1)+1];
%     dofBCs_m_bottom = [dofBCs_m, dof_m*(i-1)+1, dof_m*(i-1)+2];
% end
...
% for i = nx+1:nx+1:(ny+1)*(nx+1)
% %     plot(crds(i,1),crds(i,2),'r.')
%     dofBCs_m = [dofBCs_m, dof_m*(i-1)+2];
%     dofBCs_m_top = [dofBCs_m, dof_m*(i-1)+1];
% end
